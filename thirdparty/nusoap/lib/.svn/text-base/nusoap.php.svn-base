	...
	function serializeType($name, $type, $value, $use='encoded', $encodingStyle=false, $unqualified=false)
	{
		...
		if ($phpType == 'struct') {
			...
			if (is_array($value)) {
				...
				// @TODO This is hacked in to force already constructed XML strings into place to work with 
				// sharepoint -- marcus@silverstripe.com.au
				if (count($value) == 1 && isset($value[0])) { $xml .= $value[0]; }
				$xml .= "</$elementName>";
			} else {
